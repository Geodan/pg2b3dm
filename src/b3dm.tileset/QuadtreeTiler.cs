...
public class QuadtreeTiler
{
    ...
    public List<Tile> GenerateTiles(BoundingBox bbox, Tile tile, List<Tile> tiles, int lod=0, bool addOutlines=false, double areaTolerance = 0.01)
    {
        ...
        if (numberOfFeatures == 0) {
            ...
            for (var x = 0; x < 2; x++) {
                for (var y = 0; y < 2; y++) {
                    ...
                    GenerateTiles(bboxQuad, new_tile, tiles, lod, addOutlines, areaTolerance);
                }
            }
        }
        else {
            ...
            if (!skipCreateTiles) {
                ...
                var bytes = B3dmWriter.ToB3dm(geometries, copyright, addOutlines, areaTolerance);
                tile.Lod = lod;
                ...
                if(lodColumn!=String.Empty) {
                    if (lod < lods.Max()){
                        ...
                        var lodNextTiles = GenerateTiles(bbox, t2, new List<Tile>(), nextLod, addOutlines, areaTolerance);
