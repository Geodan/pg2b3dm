...
public class QuadtreeTiler
{
    ...
    public List<Tile> GenerateTiles(BoundingBox bbox, Tile tile, List<Tile> tiles, bool addOutlines = false, double areaTolerance = 0.01, string defaultColor = "#FFFFFF", string defaultMetallicRoughness = "#008000", bool doubleSided = true, bool createGltf = false)
    {
        var where = (query != string.Empty ? $" and {query}" : String.Empty);

        if (lodColumn != String.Empty && lods.Contains(tile.Z)) {
            var lodquery = LodQuery.GetLodQuery(lodColumn, tile.Z);
        ...
        if (numberOfFeatures > 0 && lods.Contains(tile.Z)) {
            ...
            if (!skipCreateTiles) {
                ...
                var bytes = TileWriter.ToTile(geometries, copyright, addOutlines, areaTolerance, tile.Z == 0? "#0000FF": "#008000", defaultMetallicRoughness, doubleSided, createGltf);
                // tile.Lod = lod;
        ...
        if (tile.Z < lods.Max()) {
            ...
            for (var x = 0; x < 2; x++) {
                for (var y = 0; y < 2; y++) {
                    ...
                    GenerateTiles(bboxQuad, new_tile, tiles, addOutlines, areaTolerance, defaultColor, defaultMetallicRoughness, doubleSided, createGltf);
                }
            }
        }
