...
public class QuadtreeTiler
{
    ...
    public QuadtreeTiler(string connectionString, InputTable inputTable, StylingSettings stylingSettings, int maxFeaturesPerTile, double[] translation, string outputFolder, List<int> lods, string copyright = "", bool skipCreateTiles = false, bool useImplicitTiling = false, SortBy sortBy = SortBy.AREA)
    {
    ...
    private void CreateTile(BoundingBox bbox, Tile tile, List<Tile> tiles, string where, int lod, bool createGltf, bool keepProjection, HashSet<string> processedGeometries)
    {
        ...
        var geometries = GeometryRepository.GetGeometrySubset(conn, inputTable.TableName, inputTable.GeometryColumn, tile.BoundingBox, source_epsg, target_srs, inputTable.ShadersColumn, inputTable.AttributeColumns, where, inputTable.RadiusColumn, processedGeometries, sortBy: sortBy, keepProjection: keepProjection, idColumn: inputTable.IdColumn, includeTextures: inputTable.UseTexturePipeline);
