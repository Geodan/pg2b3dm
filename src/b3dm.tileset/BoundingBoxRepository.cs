...
public static class BoundingBoxRepository
{
    ...
    {
        var sqlBounds = $"SELECT st_xmin(geom1),st_ymin(geom1), st_xmax(geom1), st_ymax(geom1) FROM (select ST_3DExtent({geometry_column}) as geom1 from {geometry_table}) as t";
